...
Pooling, in its most general form, uses a \emph{pooling operator} to reduce
information in fixed regions to an aggregate value \cite{Sun:2017}, hence
lowering the complexity and number of features with which the DNN is expected to
work. In tasks relating to computer vision, it is typical to employ
\emph{max-pooling}, whereby the pooling operator aggregates a region by
selecting its maximal value \cite{Nagi:2011}. A $2 \times 2$ region was
empirically chosen.
